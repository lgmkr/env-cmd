...
const rcFilenames = ['.env-cmdrc', '.env-cmdrc.json']
...
function EnvCmd(args) {
  ...
  if (fs.existsSync(rcFileLocation)) {
    ...
    parsedEnv = UseCmdLine({
      envFile: parsedArgs.envFile,
      useFallback: parsedArgs.useFallback
    })
  }
  ...
  SIGNALS_TO_HANDLE.forEach(signal => {
    terminateSpawnedProcessFuncHandlers[signal] = TerminateSpawnedProc.bind(
      sharedState,
      proc,
      signal
    )
...
function UseRCFile(options) {
  ...
  in .rc file:
    ${rcFileLocation}`)
    throw new Error(
      `Missing environment ${options.envFile} in .env-cmdrc file.`
    )
  }
...
function UseCmdLine(options) {
  ...
  if (!file && options.useFallback) {
    try {
      ...
      throw new Error(
        `Error! Could not find fallback file or read env file at ${envFilePathDefault}`
      )
    }
  }
